...
const DESKTOP_WEBP = path.join(ROOT, "images", "hero-img.webp");
...
const dims = execSync(`sips -g pixelWidth -g pixelHeight "${SRC}"`, {
  encoding: "utf8",
})
  .split("\n")
  .filter(Boolean)
...
execSync(`cwebp -q ${WEBP_Q} "${SRC}" -o "${DESKTOP_WEBP}"`, {
  stdio: "inherit",
});
execSync(
...
execSync(
  `cwebp -q ${WEBP_Q} -resize ${MOBILE_WIDTH} 0 "${SRC}" -o "${MOBILE_WEBP}"`,
  { stdio: "inherit" },
);
execSync(
...
for (const file of [DESKTOP_WEBP, MOBILE_JPG, MOBILE_WEBP, OG_IMAGE, SRC]) {
